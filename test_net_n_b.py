...
from model.utils.net_utils import save_net, load_net, vis_detections,mask_detections,mask_detections11
from model.faster_rcnn.vgg16_n import vgg16
from model.faster_rcnn.resnet_n_b import resnet
try:
    xrange          # Python 2
except NameError:
...
def cal_precision_recall_mae(prediction, gt):
    ...
    if prediction.shape != gt.shape:
        ...
        prediction = cv2.resize(prediction, (gt.shape[1],gt.shape[0]))
    y_test=gt.flatten()
...
def calculate_pixel_f1(pd, gt):
    if pd.shape != gt.shape:
        print("size not equal")
        pd = cv2.resize(pd, (gt.shape[1],gt.shape[0]))
    pd = pd.flatten()
    gt = gt.flatten()
...
if __name__ == '__main__':
  ...
  for i in range(num_images):
      mask_gt = cv2.imread(imdb.mask_path_at(i),0)
    #  print(imdb.mask_path_at(i))
     # ret, mask_gt = cv2.threshold(mask_gt, 127, 255, cv2.THRESH_BINARY)
      mask_gt = (mask_gt / 255).astype(int)
      ...
      for j in xrange(1, imdb.num_classes):
          ...
          #  keep = nms(cls_boxes[order, :], cls_scores[order], cfg.TEST.NMS)
           # cls_dets = cls_dets[keep.view(-1).long()]
            ...
            cls_dets = keep 
            if vis:
              mask = mask_detections11(mask, imdb.classes[j], cls_dets.cpu().numpy(), 0.0)         
    ...
    #  mask = (mask / 255.0).astype(np.float32)
      auc_score = cal_precision_recall_mae(mask, mask_gt)
