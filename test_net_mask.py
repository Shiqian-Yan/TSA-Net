...
from model.utils.net_utils import save_net, load_net, vis_detections,mask_detections
...
def cal_precision_recall_mae(prediction, gt):
    ...
    if prediction.shape != gt.shape:
        gt = cv2.resize(prediction, (prediction.shape[1],prediction.shape[0]))
    y_test=gt.flatten()
...
def calculate_pixel_f1(pd, gt):
    if pd.shape != gt.shape:
        gt = cv2.resize(pd, (pd.shape[1],pd.shape[0]))
    pd = pd.flatten()
    gt = gt.flatten()
...
if __name__ == '__main__':
  ...
  for i in range(num_images):
      ...
      with torch.no_grad():
              ...
              im_data_n.resize_(data[1].size()).copy_(data[1])
      ...
      rois_label = fasterRCNN(im_data, im_data_n,im_info, gt_boxes, num_boxes)
      ...
      for j in xrange(1, imdb.num_classes):
          ...
          if inds.numel() > 0:
            ...
            if vis:
              mask = mask_detections(mask, imdb.classes[j], cls_dets.cpu().numpy(), 0.8)         
      ...
      precision, recall,auc_score = cal_precision_recall_mae(mask, mask_gt)
     # f1 = cal_fmeasure(precision, recall)
      #f1=np.max(np.array(f1))
      f1, p, r = calculate_pixel_f1(mask, mask_gt)
     # print('F1 score per image：',f1)
      fscores.append(f1)
     # print('AUC score per image：', auc_score)
  ...
  print('Mean F1score = {:.4f}'.format(np.mean(fscores)))
