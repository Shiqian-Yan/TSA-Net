...
def modify_xml(filename, savefile, xmin, ymin, xmax, ymax,label):
    ...
    new_obj.append(create_node('name', {}, label))
...
if __name__ == '__main__':
    ...
    while count < DATASET_SIZE:
        ...
        if count < len(image_index):
            new_img.save(os.sep.join([save_image_path,image_index[img_idx]+args.imdb + '.jpg']))
            modify_xml(os.sep.join([image_annotation_path, image_index[img_idx] + '.xml']),
                   os.sep.join([save_annotation_path, image_index[img_idx]+args.imdb + '.xml']),
                   loc_x+1, loc_y+1, loc_x+max_x - min_x, loc_y+max_y - min_y,label)
        else:
            new_img.save(os.sep.join([save_image_path, '00'+str(9963+count-len(image_index)+1)+args.imdb + '.jpg']))
            modify_xml(os.sep.join([image_annotation_path, image_index[img_idx] + '.xml']),
                       os.sep.join([save_annotation_path, '00'+str(9963+count-len(image_index)+1) +args.imdb+ '.xml']),
                       loc_x + 1, loc_y + 1, loc_x + max_x - min_x, loc_y + max_y - min_y,label)
        count += 1
