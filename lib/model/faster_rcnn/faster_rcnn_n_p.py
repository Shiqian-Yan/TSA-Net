...
class _fasterRCNN(nn.Module):
    ...
    def __init__(self, classes, class_agnostic,phi=0):
        ...
        self.mylayer = nn.Linear(4096*4*4,4096*4)
        # self.RCNN_roi_pool = _RoIPooling(cfg.POOLING_SIZE, cfg.POOLING_SIZE, 1.0/16.0)
        # self.RCNN_roi_align = RoIAlignAvg(cfg.POOLING_SIZE, cfg.POOLING_SIZE, 1.0/16.0)

        self.RCNN_roi_pool = ROIPool((cfg.POOLING_SIZE, cfg.POOLING_SIZE), 1.0 / 16.0)
        self.RCNN_roi_align = ROIAlign((cfg.POOLING_SIZE, cfg.POOLING_SIZE), 1.0 / 16.0, 0)
        self._head    = cbam_block(4096)
        self.phi  = phi
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes):
        ...
        bipooled_feat = torch.cat([pooled_feat, pooled_feat_n], dim=1)
        bipooled_feat = self._head(bipooled_feat)
        bipooled_feat = bipooled_feat.mean(3).mean(2)
        bipooled_feat = bipooled_feat.reshape(-1,4096)
